...
const unsigned int NUM_CASCADES = 3;
...
void CalcOrthoProjs(ShadowOrthoProjInfo* m_shadowOrthoProjInfo, float* m_cascadeEnd, float ar, float fov, glm::mat4 view, glm::mat4 lightView);
void shadowMapPass(Shader simpleDepthShader, CascadedShadowMapFBO csmFbo, glm::vec3 lightPos, ShadowOrthoProjInfo* m_shadowOrthoProjInfo);
...
int main()
{
	...
	// -------------------------
	Shader shader("shader/3.1.3.shadow_mapping.vs", "shader/3.1.3.shadow_mapping.fs");
	...
	CascadedShadowMapFBO csmFBO;
	csmFBO.Init(SHADOW_WIDTH, SHADOW_HEIGHT);
	/*
	...
	*/
	...
	for (uint i = 0; i < NUM_CASCADES; i++) {
		shader.setInt("shadowMap[" + to_string(i) + "]", i);
	}
	shader.setInt("diffuseTexture", NUM_CASCADES);
	debugDepthQuad.use();
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (uint i = 0; i <= NUM_CASCADES; i++) {
			cascadeEnd[i] = near_plane + i * (far_plane - near_plane) / NUM_CASCADES;
		}
		ShadowOrthoProjInfo sops[NUM_CASCADES];
		CalcOrthoProjs(sops, cascadeEnd, (float)SCR_WIDTH / (float)SCR_HEIGHT, camera.Zoom, view, lightView);
		shadowMapPass(simpleDepthShader, csmFBO, lightPos, sops);
		...
		for (uint i = 0; i < NUM_CASCADES; i++) {
			shader.setFloat("gCascadeEndClipSpace[" + to_string(i) + "]", cascadeEnd[i+1]);
			ShadowOrthoProjInfo sop = sops[i];
			lightProjection = glm::ortho(sop.l, sop.r, sop.b, sop.t, sop.n, sop.f);
			lightSpaceMatrix = lightProjection * lightView;
			shader.setMat4("lightSpaceMatrix[" + to_string(i) + "]", lightSpaceMatrix);
		}
		csmFBO.BindForReading();

		glActiveTexture(GL_TEXTURE0 + NUM_CASCADES);
		...
		glBindTexture(GL_TEXTURE_2D, csmFBO.m_shadowMap[0]);
		renderQuad();
...
CascadedShadowMapFBO::~CascadedShadowMapFBO()
{
	...
	glDeleteTextures(NUM_CASCADES, m_shadowMap);
}
...
void shadowMapPass(Shader simpleDepthShader, CascadedShadowMapFBO csmFbo, glm::vec3 lightPos, ShadowOrthoProjInfo* m_shadowOrthoProjInfo) {
	...
	for (uint i = 0; i < NUM_CASCADES; i++) {
		csmFbo.BindForWriting(i);
		glClear(GL_DEPTH_BUFFER_BIT);
		ShadowOrthoProjInfo sop = m_shadowOrthoProjInfo[i];
		lightProjection = glm::ortho(sop.l, sop.r, sop.b, sop.t, sop.n, sop.f);
		lightSpaceMatrix = lightProjection * lightView;
